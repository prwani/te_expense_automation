...
def _env_diagnostics() -> Dict[str, Any]:
    """Return a diagnostics dict for required env vars (no secrets)."""
    return {
        "has_storage_account": bool(os.getenv("AZURE_STORAGE_ACCOUNT_NAME")),
        "has_storage_container": bool(os.getenv("AZURE_STORAGE_CONTAINER_NAME")),
        "has_aoai_endpoint": bool(os.getenv("AZURE_OPENAI_ENDPOINT") or os.getenv("ENDPOINT_URL")),
        "has_aoai_deployment": bool(os.getenv("AZURE_OPENAI_DEPLOYMENT") or os.getenv("DEPLOYMENT_NAME")),
...
def extract_invoice_line_items(blob_name: str) -> str:
    ...
    # Guard: if container is None fall back to default 'receipts' so function can attempt; fetch_blob_to_data_uri will still validate
    container_name = CONTAINER_NAME or os.getenv("AZURE_STORAGE_CONTAINER_NAME") or "receipts"
    ...
    except Exception as e:  # Surface concise error upstream
        # Encode a small JSON object so caller can detect and branch
        return (
            '{"error": "aoai_request_failed", "message": ' +
            json.dumps(str(e).split('\n')[0]) + ', "diagnostics": ' + json.dumps(_env_diagnostics()) + '}'
        )
